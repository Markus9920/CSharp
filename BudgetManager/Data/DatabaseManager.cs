...
namespace BudgetManager.Data
{
    public static class DatabaseManager
    {
        ...
        public static DataTable GetAllExpenses(int userId, string tableName)
        {
            if (tableName != recurringExpenseTableName && tableName != otherExpensesTableName)
            {
                throw new ArgumentException("Invalid table name", nameof(tableName));
            ...
            using (var connection = new SqliteConnection(dataBase))
            {
                connection.Open();
                var command = connection.CreateCommand();
                command.CommandText = $"SELECT expenseID, description, cost FROM {tableName} WHERE userID = $userID";
                command.Parameters.AddWithValue("$userID", userId);
