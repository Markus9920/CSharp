...
"JwtSettings": {
  ...
  public class TokenService
  {
    ...
    private readonly TimeSpan expMinutes = TimeSpan.FromMinutes(0.5);
    private readonly TimeSpan expDays = TimeSpan.FromDays(15);
    ...
    public ClaimsPrincipal? ValidateAccesToken(string token) //method that returns identity information
    {
    ...
    public ClaimsPrincipal? ValidateRefreshToken(string token) //method that returns identity information
    {
      //object for reading the token
      JwtSecurityTokenHandler handler = new JwtSecurityTokenHandler();
      byte[] key = Encoding.UTF8.GetBytes(_key); //byte array of the key, that is used to sign the token
      ...
      try
      {
        ...
        //finds the user id from validated user, or it also can be null if nothing is found
    ...
    //if any token exists
    public bool AnyTokensExists(HttpRequest request) => AccessTokenExists(request) || RefreshTokenExists(request);
    //Get tokens
    ...
    //these check if token is found
    public bool AccessTokenExists(HttpRequest request) => request.Cookies.ContainsKey(accessTokenName);
